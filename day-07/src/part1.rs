...
pub fn hand(input: &str) -> IResult<&str, Hand> {
    ...
    let counts = cards.iter().fold(HashMap::with_capacity(5), |mut map, v| {
        map.entry(v).and_modify(|c| *c += 1).or_insert(1u8);
        map
    });

    let hand_kind = match counts.values().len() {
        5 => Kind::HighCard,
        4 => Kind::OnePair,
        3 => counts
            .values()
        ...
        2 => counts
            ...
            .expect("key count len 2 had invalid combination"),
        1 => Kind::FiveOfAKind,
        _ => panic!("Invalid combination of cards"),
    };
